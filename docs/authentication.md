...
- `build_app_jwt` (`src/codereeve/chain/app_auth.py`) signs a JWT with a 9-minute TTL and a 60-second `iat` backdate for clock skew.
...
**Consumed via:** `Identity.APP` (`src/codereeve/chain/identity.py`) and `gh_env()` / `app_auth.py`, for daemon-privileged operations only:

- Feature-branch push (`_authed_git_push`, `src/codereeve/chain/daemon/push_probe.py`)
...
**Validated by:** `validate_daemon_token()` (`src/codereeve/_auth.py`) — a type-gate that rejects anything not prefixed `ghs_`. Runs as gate **G3a** at daemon startup (`src/codereeve/chain/reconcile.py`, fatal on failure) and again in `src/codereeve/chain/cli.py`.
...
|---|---|---|
| `contents` | `write` | Push `feature/*` and `codereeve/*` branches; read repo files |
...
- `tests/chain/test_identity_spawn_guard.py` AST-walks every subprocess spawn under `src/codereeve/chain/` and fails closed on any spawn that omits an explicit identity — no call site can silently inherit an ambient token.
...
`CODEREEVE_GITHUB_APP_KEY_PROVIDER` is required and accepts exactly `bws` or `file`. The selected
...
| `file` | `CODEREEVE_GITHUB_APP_PRIVATE_KEY_FILE=<absolute-path>` | None for the App key |
...
`CODEREEVE_GITHUB_APP_KEY_PROVIDER=bws` and retain `BWS_PEM_SECRET_ID`. File deployments set
`CODEREEVE_GITHUB_APP_KEY_PROVIDER=file`, set `CODEREEVE_GITHUB_APP_PRIVATE_KEY_FILE`, and remove
...
supplied as `GH_TOKEN` to the daemon for the standard `codereeve hook before-run` worker hook. The
...
- **Vault fetch:** set `BWS_GH_TOKEN_SECRET_ID` in `${CODEREEVE_PROJECT_ROOT}/.codereeve/config.env`; `bootstrap_secrets()` fetches the PAT from Bitwarden at startup and retains it by value for the worker hook without writing it to `os.environ`.
...
`/etc/codereeve/worker.env` (owner `root`, mode `0600`) with a `GH_TOKEN` assignment,
then reference it from a service drop-in with
`EnvironmentFile=/etc/codereeve/worker.env`. This supplies the required worker PAT
alongside the separately mounted App PEM. The installer does not create this file or
drop-in. Never put the token in a committed unit, `.codereeve/config.env`, or any repository
file. See the complete [BWS-free service example](smoke-test-daemon.md#manual--reference).

**Validated by:** `validate_github_token()` (`src/codereeve/_auth.py`) — reads `GH_TOKEN`/`GITHUB_TOKEN`, rejects classic `ghp_` PATs and anything not prefixed `github_pat_`, then runs a live `gh api user` capability self-test (retried up to `_MAX_RETRIES` = 2 times on transient failures — rate-limits, gateway errors, DNS/TLS — before failing closed; permanent 401/403 failures raise immediately). This runs at the top of the `codereeve hook before-run` handler (`src/codereeve/before_run.py`), i.e. once per worker turn, not at daemon startup — a **different gate, on a different credential, in a different execution context** than `validate_daemon_token()` above. Known limitation: fine-grained PATs expose no scope-introspection API (unlike classic PATs' `X-OAuth-Scopes` response header), so this gate verifies token *type* and *reachability* only, not the exact granted permission set — that verification is the operator's responsibility at mint time.
...
**Consuming code:** `src/codereeve/chain/app_auth.py` (`bootstrap_secrets`,
`build_installation_token_provider`, `main`).

**Provisioning:** [docs/repository-onboarding.md §3](repository-onboarding.md#3-bws_access_token--export-only-when-bws-is-configured) covers conditionally exporting `BWS_ACCESS_TOKEN` for an interactive session, and the `/etc/codereeve/secrets.env` (mode `600`) pattern for a persistent BWS-backed deployment.
...
- Gate **G3b** (`src/codereeve/chain/reconcile.py`) checks `ANTHROPIC_API_KEY` is absent at every daemon startup — fatal, critical alert, `sys.exit(1)` if present. A non-empty key means per-token billing is active, which contradicts the subscription-only cost model.
- Gate **G3c** (`src/codereeve/chain/reconcile.py`) checks the OAuth credential file is present and readable — structural check only (presence + `open()` succeeds); contents are never read, decoded, or logged. Fatal if absent or unreadable.
...
GitHub tokens plus `CODEREEVE_GITHUB_APP_KEY_PROVIDER`, `CODEREEVE_GITHUB_APP_PRIVATE_KEY_FILE`,
