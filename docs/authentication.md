...
**This is a narrower credential than the GitHub App, not an equivalent one.** GitHub does not allow `administration` or `checks` to be granted to a fine-grained PAT at all (`checks` became App-only as of mid-2026; `administration` is App-only by design) — see harness-design.md §12: "a fine-grained PAT cannot hold the `checks` permission the daemon's CI reads need, and a feature-branch push to a ruleset-protected branch requires the bypass actor to be the GitHub App itself, not a PAT" (issue #220). Consequences:

- CI status is read from the Actions API (`repos/{owner}/{repo}/actions/runs` + `.../jobs`, `src/baton_harness/chain/merge.py`) instead of the Checks API, specifically to work within what a PAT can be granted (#121).
- The PAT cannot be the ruleset bypass actor for `feature/*` pushes or satisfy the `administration` preflight read — those operations require `Identity.APP`.
