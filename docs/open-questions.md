...
- **Container OOM / ungraceful exit** → **resolved (#40).** A `.baton-harness/daemon.alive` marker is written at startup and cleared in the `run_daemon` `finally` block on graceful shutdown (`daemon.py:1585-1596`). If the marker is present on the next boot, the prior run ended ungracefully (likely OOM-kill or hard crash) — `alert(severity="critical")` fires (`reconcile.py:138-154`). The harness cannot self-report an uncatchable SIGKILL; next-boot detection is the tractable notification.
- **Credential corruption / missing credentials** → **resolved (#40).** `validate_github_token()` (GitHub token) and an `ANTHROPIC_API_KEY` presence probe both run at daemon startup (`reconcile.py:105-132`). Either failure emits `alert(severity="critical")` then calls `sys.exit(1)` — the daemon exits non-zero rather than starting and silently failing on every worker dispatch. Scope confirmed as env-injected tokens only (no credential file or volume; see plan §2).
