...
**Orchestrator = custom always-on daemon [implemented].** A persistent CodeReeve process that never exits between work units or on a block (`src/codereeve/chain/daemon.py`). It watches for ready work units, builds and schedules the DAG (`graphlib.TopologicalSorter`), owns the `codereeve/<slug>` branch lifecycle and ready-for-review `codereeve/<slug> → main` PR, drives Slack escalation, and parks or resumes sub-trees.
