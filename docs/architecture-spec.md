...
**Merge boundary (slice 3b):** worker-identity merges into `main` and `feature/<slug>` are denied by a GitHub Repository Ruleset. The daemon's GitHub App installation is the sole bypass actor on `feature/<slug>`; only the repo admin role bypasses on `main`. See issue #157 and the merge PR for the design.
