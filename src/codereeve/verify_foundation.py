...
from codereeve.provenance import ProvenanceError, validate_provenance
...
def verify_installed(forbidden_distributions: frozenset[str]) -> None:
    ...
    try:
        distribution = metadata.distribution("codereeve")
        package_file = Path(codereeve.__file__ or "")
    ...
    try:
        with ZipFile(wheel) as archive:
            ...
            for resource_name in RESOURCE_NAMES:
                expected = f"codereeve/resources/{resource_name}"
    ...
    try:
        ...
        if path.suffix == ".whl":
            with ZipFile(path) as wheel:
                ...
                canonical = "codereeve/build_provenance.json"
            ...
            with tarfile.open(path, "r:gz") as source:
                ...
                canonical = f"{root}/src/codereeve/build_provenance.json"
