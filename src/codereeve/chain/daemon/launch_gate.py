...
``mock.patch("codereeve.chain.daemon.X", ...)`` in existing tests
...
from codereeve.chain import recovery as _recovery_mod
...
def reconstruct(
    ...
    *,
    ...
    (``"codereeve.chain.daemon.reconstruct"``) or the upstream
    symbol (``"codereeve.chain.recovery.reconstruct"``) and both
