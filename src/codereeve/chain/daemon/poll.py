...
``mock.patch("codereeve.chain.daemon.X", ...)`` /
...
``"codereeve.chain.daemon._poll_and_run"`` and expects ``run_daemon``
...
``codereeve.chain.reconcile`` rather than reached via
...
(``codereeve.chain.daemon.poll.reconcile_startup``), including the
...
import codereeve.chain.daemon as _daemon_mod
from codereeve.chain.app_auth import InstallationTokenSource, gh_env
from codereeve.chain.failure_tally import FailureTally
from codereeve.chain.heartbeat import LivenessState, run_heartbeat_loop
...
from codereeve.chain.reconcile import reconcile_startup
from codereeve.chain.recovery import scan_orphan_worktrees
from codereeve.chain.redispatch import RedispatchTally
...
# Hard-coded: preserves the pre-split "codereeve.chain.daemon" logger
...
_log = logging.getLogger("codereeve.chain.daemon")
    ...
    *,
    ...
    for orphan in orphans_raw:
        ...
        if ms_info:
            ...
            from codereeve.chain.branches import feature_branch_name
