...
def test_reconcile_preserves_native_order_without_repeating_live_gate(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Reconcile keeps native guards before markers without doctor reruns."""
    reconcile = _import_reconcile()
    monkeypatch.setenv("GH_TOKEN", _INSTALLATION_TOKEN)
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    events = []
    ...
    def helpers() -> list[str]:
        """Record credential guard before persistent markers."""
        assert not marker.exists()
        events.append("helper")
        return ["test-helper"]

    def processes() -> list[int]:
        """Verify process recovery follows marker creation."""
        assert marker.exists()
    ...
    with (
        ...
        asyncio.run(
            reconcile.reconcile_startup(
                [_make_repo_cfg(tmp_path)], _make_obs(tmp_path), runlog=None
            )
        )
    assert events == ["token", "helper", "processes"]
