...
_NEW_VAR = "CODEREEVE_TEST_NEW_VAR"
_OVERRIDE_VAR = "CODEREEVE_TEST_OVERRIDE_VAR"
...
def _run_config(
    ...
    wrapper.write_text(
        ...
        'source "$1"\n'
        'bash -c \'printf "%s\\n" '
        '"${CODEREEVE_TEST_NEW_VAR-}" "${CODEREEVE_TEST_OVERRIDE_VAR-}"\'\n',
        encoding="utf-8",
        newline="\n",
    )
...
def test_new_plain_config_variable_is_visible_to_grandchild(
    ...
    proc = _run_config(tmp_path, "CODEREEVE_TEST_NEW_VAR=hello-from-config\n")
...
def test_operator_override_is_restored_and_visible_to_grandchild(
    ...
    proc = _run_config(
        tmp_path,
        "CODEREEVE_TEST_OVERRIDE_VAR=value-from-config\n",
